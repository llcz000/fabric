...
const SAMPLE_COLUMNS: ColumnDefinition[] = [
  ...
  { header: '品名', width: 12, value: (item) => item.product_name ?? '' },
  { header: '成分', width: 14, value: (item) => item.composition ?? '' },
  { header: '克重', width: 7, value: (item) => item.weight ?? '' },
...
export function buildDocumentWorkbook(input: DocumentWorkbookInput): ExcelJS.Workbook {
  ...
  styledCell(sheet.getCell(1, 3), company.company_name ?? '', 16, true, 'right');
  styledCell(sheet.getCell(2, 3), `地址：${company.address ?? ''}    电话：${company.phone ?? ''}`, 9, false, 'right');
  sheet.getRow(1).height = 24;
  sheet.getRow(2).height = 18;

  sheet.mergeCells(3, 1, 3, lastColumn);
  styledCell(sheet.getCell(3, 1), title, 14, true, 'center');
  sheet.getRow(3).height = 25;

  sheet.mergeCells(4, 1, 4, splitColumn);
  sheet.mergeCells(4, splitColumn + 1, 4, lastColumn);
  styledCell(sheet.getCell(4, 1), `NO：${order.order_no ?? ''}`, 10, false, 'left');
  styledCell(sheet.getCell(4, splitColumn + 1), `日期：${String(order.order_date ?? '').slice(0, 10)}`, 10, false, 'right');
  sheet.mergeCells(5, 1, 5, lastColumn);
  styledCell(sheet.getCell(5, 1), `收货单位：${order.receiving_unit ?? ''}`, 10, false, 'left');

  const headerRowNumber = 6;
  const headerRow = sheet.getRow(headerRowNumber);
  columns.forEach((column, index) => {
    const cell = headerRow.getCell(index + 1);
    styledCell(cell, column.header, 9, true, 'center');
  ...
  items.forEach((item, itemIndex) => {
    ...
    columns.forEach((column, columnIndex) => {
      ...
      styledCell(cell, column.value(item), 9, false, 'center');
      cell.alignment = { ...cell.alignment, wrapText: true, shrinkToFit: true };
      cell.border = THIN_BORDER;
  ...
  styledCell(sheet.getCell(termsRow, 1), `备注条款：${company.default_terms ?? ''}`, 9, false, 'left');
  ...
  sheet.getRow(termsRow).height = 28;
  ...
  styledCell(sheet.getCell(footerRow, 1), `开单人签字：${order.sign_person ?? ''}`, 10, false, 'left');
  sheet.mergeCells(footerRow + 1, 1, footerRow + 1, qrStartColumn - 1);
  styledCell(sheet.getCell(footerRow + 1, 1), `收货人签字：${order.receiver ?? ''}    电话：${company.phone ?? ''}`, 10, false, 'left');
  for (let rowNumber = footerRow; rowNumber <= footerRow + 3; rowNumber++) sheet.getRow(rowNumber).height = 18;
  ...
  styledCell(sheet.getCell(footerRow + 3, qrStartColumn), '微信收款', 8, false, 'center');
  sheet.mergeCells(footerRow + 3, lastColumn - 1, footerRow + 3, lastColumn);
  styledCell(sheet.getCell(footerRow + 3, lastColumn - 1), '支付宝收款', 8, false, 'center');
  ...
  sheet.pageSetup = {
    ...
    margins: { left: 0.4, right: 0.4, top: 0.25, bottom: 0.25, header: 0, footer: 0 },
  };
...
function addSummaryRow(sheet: ExcelJS.Worksheet, rowNumber: number, lastColumn: number, left: string, right: string): void {
  ...
  for (const [column, value] of [[1, left], [half + 1, right]] as const) {
    ...
    styledCell(cell, value, 9, true, 'left');
    cell.border = THIN_BORDER;
  }
