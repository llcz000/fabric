...
export default function ProductLibrary({ onBack }: Props) {
  // Data
  ...
  // Filters (tag-based multi-select)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const file of pendingFiles) {
        const { thumbnail, full } = await processImageUpload(file);
  ...
  const openLightbox = async (productId: string, startIndex: number) => {
  ...
  const handleSimilarSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // data.results: [{ productId, imageId, distance }]
      // Highlight matching products
      const matchIds = new Set<string>(data.results?.map((r: any) => r.productId) || []);
      setSelectedIds(new Set()); // clear selection
      // Set itemNo filter to matching product itemNos
  ...
  const ThumbnailCell: React.FC<{ productId: string }> = ({ productId }) => {
    ...
    useEffect(() => {
      ...
      getImages(productId).then(imgs => {
        if (cancelled) return;
        setThumbs(imgs.slice(0, 3).map(i => ({ id: i.id, url: URL.createObjectURL(i.thumbnail) })));
        setLoaded(true);
      });
      ...
      <div className="flex gap-1">
        {thumbs.map((t, i) => (
          <img
            key={t.id}
            src={t.url}
            className="w-14 h-14 object-cover rounded border border-slate-200 cursor-pointer hover:opacity-80 transition-opacity"
            onClick={(ev) => { ev.stopPropagation(); openLightbox(productId, i); }}
            alt="花型缩略图"
          />
        ))}
        {thumbs.length >= 3 && (
          <div
            className="w-14 h-14 bg-slate-100 rounded border border-slate-200 flex items-center justify-center cursor-pointer text-xs font-bold text-slate-500"
            onClick={(ev) => { ev.stopPropagation(); openLightbox(productId, 2); }}
          >
            +{thumbs.length}
          </div>
        )}
  ...
  return (
    <div className="space-y-4">
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-slate-900 text-white px-4 py-2.5 rounded-xl shadow-lg text-sm font-medium animate-pulse">
          {toast}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xs p-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            type="file" accept="image/*" className="hidden" ref={similarInputRef}
            onChange={handleSimilarSearch}
          />
          <button
            type="button"
          ...
          >
            ...
            导出{selectedIds.size > 0 ? `选中(${selectedIds.size})` : '全部'}
          </button>
          <button
            type="button"
            onClick={openCreate}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xs p-3 space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1 flex-wrap">
            ...
            {itemNoTags.map(t => (
              <span key={t} className="inline-flex items-center gap-0.5 bg-sky-50 text-sky-700 text-xs px-2 py-0.5 rounded-full">
                {t}
                <button onClick={() => removeTag(t, itemNoTags, setItemNoTags)} className="hover:text-sky-900 cursor-pointer"><X className="w-3 h-3" /></button>
              </span>
            ))}
            <input
              type="text"
              ...
              placeholder="输入后回车..."
              className="w-24 text-xs px-2 py-1 border border-slate-200 rounded-md outline-none focus:border-sky-300"
            />
          </div>
          {/* Product Name filter */}
          <div className="flex items-center gap-1 flex-wrap">
            <span className="text-xs text-slate-400">品名:</span>
            {productNameTags.map(t => (
              <span key={t} className="inline-flex items-center gap-0.5 bg-sky-50 text-sky-700 text-xs px-2 py-0.5 rounded-full">
                {t}
                <button onClick={() => removeTag(t, productNameTags, setProductNameTags)} className="hover:text-sky-900 cursor-pointer"><X className="w-3 h-3" /></button>
              </span>
            ))}
            <input
              type="text"
          ...
          {hasFilters && (
            <button
              onClick={() => { setItemNoTags([]); setProductNameTags([]); }}
          ...
          <>
                    ...
                    <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-2 px-2">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            onClick={() => openEdit(p)}
                            className="p-1.5 hover:bg-slate-100 rounded-lg cursor-pointer text-slate-400 hover:text-sky-600 transition-colors"
                            title="编辑"
                          >
                          ...
                          <button
                            ...
                            className="p-1.5 hover:bg-red-50 rounded-lg cursor-pointer text-slate-400 hover:text-red-500 transition-colors"
                            title="删除"
                          >
            ...
            {selectedIds.size > 0 && (
              <div className="border-t border-slate-200 bg-sky-50/30 px-4 py-2 flex items-center gap-3">
                ...
                <button onClick={handleExport} className="text-xs text-sky-600 hover:text-sky-800 font-semibold cursor-pointer">
                  <Download className="w-3 h-3 inline mr-1" />导出选中
                </button>
              </div>
            )}
          </>
      ...
      {editModal && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-10 pb-10 overflow-auto" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 space-y-4">
            <h3 className="text-lg font-bold text-slate-800">{editingProduct.id && products.find(p => p.id === editingProduct.id) ? '编辑产品' : '新增产品'}</h3>

            {/* Text fields */}
            <div className="grid grid-cols-2 gap-3">
              <label className="space-y-1">
                <span className="text-xs text-slate-500 font-semibold">货号 *</span>
                <input type="text" value={editingProduct.itemNo}
                  onChange={e => setEditingProduct({ ...editingProduct, itemNo: e.target.value })}
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-sky-400" />
              </label>
              <label className="space-y-1">
                <span className="text-xs text-slate-500 font-semibold">品名 *</span>
                <input type="text" value={editingProduct.productName}
                  onChange={e => setEditingProduct({ ...editingProduct, productName: e.target.value })}
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-sky-400" />
              </label>
              <label className="space-y-1">
                <span className="text-xs text-slate-500 font-semibold">成分</span>
                <input type="text" value={editingProduct.composition}
                  onChange={e => setEditingProduct({ ...editingProduct, composition: e.target.value })}
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-sky-400" />
              </label>
              <label className="space-y-1">
                <span className="text-xs text-slate-500 font-semibold">克重</span>
                <input type="text" value={editingProduct.weight}
                  onChange={e => setEditingProduct({ ...editingProduct, weight: e.target.value })}
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-sky-400" />
              </label>
              <label className="space-y-1">
                <span className="text-xs text-slate-500 font-semibold">门幅</span>
                <input type="text" value={editingProduct.width}
              ...
              <div className="flex flex-wrap gap-2">
                {pendingFiles.map((f, i) => (
                  <div key={i} className="relative">
                    <img src={URL.createObjectURL(f)} className="w-20 h-20 object-cover rounded-lg border border-sky-200" alt="" />
                    <button
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setEditModal(false)} className="px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-xl cursor-pointer">取消</button>
              <button
                onClick={handleSave}
                disabled={saving || !editingProduct.itemNo.trim() || !editingProduct.productName.trim()}
                className="px-5 py-2 bg-sky-600 hover:bg-sky-700 disabled:opacity-40 text-white rounded-xl text-sm font-semibold cursor-pointer transition-colors"
              >
                {saving ? '保存中...' : '保存'}
              </button>
            </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm mx-4 space-y-4">
            ...
            <p className="text-sm text-slate-600">确定删除产品 "<b>{deleteTarget.itemNo} {deleteTarget.productName}</b>" 及其所有花型图片吗？此操作不可撤销。</p>
          ...
          {lightboxImages.length > 1 && (
            <>
              <button
                ...
                className="absolute left-4 text-white/70 hover:text-white cursor-pointer z-10"
              >
                <ChevronLeft className="w-10 h-10" />
              </button>
              <button
                onClick={(e) => { e.stopPropagation(); setLightboxIndex(prev => prev < lightboxImages.length - 1 ? prev + 1 : 0); }}
                className="absolute right-4 text-white/70 hover:text-white cursor-pointer z-10"
              >
