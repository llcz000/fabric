...
export function ProductImageCategory({ role, label, images, uploadDisabled, canMoveTo, onUpload, onDelete, onMove, onReorder, onReplace }: {
  role: Exclude<ProductImageRole, 'legacy'>; label: string; images: ProductImageLayoutInput[];
  uploadDisabled?: boolean;
  canMoveTo?(assetId: string, role: Exclude<ProductImageRole, 'legacy'>): boolean;
  ...
  return <section className={role === 'unclassified' ? 'rounded-lg border border-amber-300 bg-amber-50 p-3' : 'rounded-lg border border-slate-200 p-3'}>
    <div className="mb-2 flex items-center justify-between"><strong>{label}</strong><label className={uploadDisabled ? 'cursor-not-allowed text-sm text-slate-400' : 'cursor-pointer text-sm text-sky-700'}>上传<input className="sr-only" type="file" accept="image/*" multiple disabled={uploadDisabled} onChange={(event) => event.target.files && onUpload?.(role, event.target.files)} /></label></div>
    <div className="space-y-2">{images.map((image) => <div key={image.assetId} className="flex items-center justify-between gap-2 rounded bg-white p-2 text-xs"><span className="truncate">{image.assetId}</span><div className="flex flex-wrap items-center gap-2">
      ...
      <select aria-label="移动分类" value={role} onChange={(event) => onMove?.(image.assetId, event.target.value as Exclude<ProductImageRole, 'legacy'>)}><option value="pattern_original" disabled={canMoveTo ? !canMoveTo(image.assetId, 'pattern_original') : false}>花型原图</option><option value="fabric_display">面料展示图</option><option value="detail">细节图</option><option value="ai_effect">AI效果图</option><option value="unclassified">待分类</option></select>
