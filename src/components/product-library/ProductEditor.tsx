...
import type { PatternTagSummary, ProductIssue, ProductItem } from '../../types';
...
export function canAssignProductImageRole(
  images: ProductImageLayoutInput[],
  assetId: string,
  role: ProductImageLayoutInput['role'],
): boolean {
  return role !== 'pattern_original'
    || !images.some((image) => image.role === 'pattern_original' && image.assetId !== assetId);
}
...
export function ProductEditor({ state, availableTags, issues = [], onChange, onSave, onClose, onUpload, onReplace, onIgnoreIssue, onReopenIssue }: {
  ...
  onIgnoreIssue?(issue: ProductIssue): void;
  onReopenIssue?(issue: ProductIssue): void;
}) {
  ...
  return <div className="fixed inset-0 z-50 overflow-auto bg-black/50 p-6"><div className="mx-auto max-w-4xl rounded-2xl bg-white p-6">
    ...
    {issues.length > 0 && <section className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3" aria-label="数据异常"><h3 className="mb-2 font-semibold text-amber-900">数据异常</h3><div className="space-y-2">{issues.map((issue) => <div key={issue.id} className="flex items-center justify-between gap-3 rounded bg-white p-2 text-sm"><div><span>{issue.message}</span>{issue.sourceRef && <span className="ml-2 text-slate-500">{issue.sourceRef}</span>}</div>{issue.status === 'ignored' ? <button type="button" onClick={() => onReopenIssue?.(issue)} className="text-amber-700">重新打开</button> : issue.status === 'open' ? <button type="button" onClick={() => onIgnoreIssue?.(issue)} className="text-slate-600">忽略</button> : <span className="text-emerald-600">已解决</span>}</div>)}</div></section>}
    <div className="grid gap-3 md:grid-cols-2">{CATEGORIES.map(([role, label]) => <ProductImageCategory key={role} role={role} label={label} images={draft.images.filter((image) => image.role === role)} uploadDisabled={!canAddProductImage(draft.images) || !canAssignProductImageRole(draft.images, '__new__', role)} canMoveTo={(assetId, targetRole) => canAssignProductImageRole(draft.images, assetId, targetRole)} onUpload={onUpload} onReplace={onReplace} onDelete={(assetId) => onChange({ ...draft, images: resequenceProductImages(draft.images.filter((image) => image.assetId !== assetId)) })} onMove={(assetId, targetRole) => { if (canAssignProductImageRole(draft.images, assetId, targetRole)) onChange({ ...draft, images: moveProductImage(draft.images, assetId, targetRole) }); }} onReorder={(assetId, direction) => { const current = draft.images.filter((image) => image.role === role).findIndex((image) => image.assetId === assetId); onChange({ ...draft, images: moveProductImage(draft.images, assetId, role, current + direction) }); }} />)}</div>
