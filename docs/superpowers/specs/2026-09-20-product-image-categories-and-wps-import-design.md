...
3. 允许有问题的 Excel 数据先导入，产品立即可见和可使用，同时以结构化异常引导人工修正。
4. 从 WPS `DISPIMG` 公式中正确提取嵌入图片，不修改源工作簿。
5. 导入过程可 dry-run、幂等续跑、逐产品事务、追溯和有条件回滚。
6. 保持现有图片资产安全、去重、变体、引用计数和回收规则。
7. 首版只管理已有 AI 效果图，不接入 AI 生图服务，但预留生成元数据。
...
- `GET /api/products`
  - 支持分页、货号、品名、`reviewStatus`、异常代码、导入批次和重复货号筛选。
  - 返回花型缩略图、分类数量、审核状态和异常数量。
- `GET /api/products/:id`
  - 返回产品字段、全部分类图片、来源摘要和未关闭异常。
- `POST /api/products`
  - 接收产品字段和 `images: [{ assetId, role, sortOrder }]`。
- `PUT /api/products/:id`
  - 修改产品字段，并在同一业务操作后重新计算相关异常。
...
筛选包括审核状态、异常代码、图片完整性、导入批次和重复货号。列表不使用其他分类图片冒充花型原图。
...
新文件先成为待保存资产。保存时一次提交产品字段和图片关联布局；失败时弹窗保持打开。取消后未关联资产由现有生命周期规则回收。
...
1. 增加角色、来源、导入批次、来源追溯和异常表结构。
2. 实现后端不变量、图片布局、异常重算和筛选接口。
...
4. 异常产品可立即使用，并能按异常筛选、逐项修正和自动完成审核。
5. 重复货号不被静默覆盖，完全重复和冲突记录按已确认规则处理。
6. 源工作簿不被修改，所有导入产品可以追溯到批次、工作表和行号。
7. WPS 单元格图片、转换、缩放、缺失媒体和附加图片均得到确定性处理。
8. dry-run、apply、断点续跑和重复执行结果可核对且幂等。
9. 生产图片资产安全、去重、鉴权、引用计数和回收规则不退化。
10. 一轮完整验收通过，最终提交不包含 `.firecrawl/`、个人配置或无关文件。
